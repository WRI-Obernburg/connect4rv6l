    ...
    <div className="relative">
      {entryState != null && (
        <motion.div
          ...
          className={cn(
            "w-20 h-20 border absolute rounded-full",
            entryState === 1
              ? "bg-red-500"
              : entryState === 2
              ? "bg-blue-500"
      ...
      <div
        className={cn(
          "w-20 h-20 border rounded-full border-gray-500 shadow hover:shadow-lg transition-shadow flex items-center justify-center",        )}
      />
    </div>
  );
}
...
function RenderBoard({ board }: { board: number[][] | null }) {
  ...
  return (
    <div className="flex flex-row gap-4 justify-center rounded-lg border-2 border-gray-300 p-4">
...
export default function GameField({ gameState }: GameFieldProps) {
