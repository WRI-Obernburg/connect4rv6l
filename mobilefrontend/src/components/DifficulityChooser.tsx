...
export default function DifficultyChooser(props: {onDifficultyChange: (difficulty: string)=>void, gameState: GameState}) {
    return <div role="radiogroup" aria-label="Schwierigkeit" className="flex w-full sm:w-auto rounded-lg border border-wri-petrol/15 bg-white/60 p-0.5">
        {LEVELS.map(level => {
            ...
            return <button key={level.value} type="button" role="radio" aria-checked={active}
                ...
                className={cn("flex-1 px-3 h-9 rounded-md text-sm font-semibold transition-colors cursor-pointer focus-visible:outline-2 focus-visible:outline-wri-cyan",
